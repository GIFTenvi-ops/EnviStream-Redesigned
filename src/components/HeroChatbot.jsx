...
export default function HeroChatbot({ onEnquire }) {
  ...
  return (
    <div className="rounded-2xl border border-white/60 bg-white/65 backdrop-blur-2xl shadow-[0_18px_50px_-24px_rgba(249,115,22,.35)] overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-3 bg-slate-900/75 backdrop-blur-xl text-white border-b border-white/10">
        <span className="w-8 h-8 grid place-items-center rounded-lg bg-[#d4a85d]/20 text-[#f0deaf] border border-white/15 shrink-0">
      ...
      <div ref={box} data-lenis-prevent className="max-h-44 overflow-y-auto px-4 py-3 space-y-2.5 bg-white/40 backdrop-blur">
        {msgs.map((m, i) => (
          <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed ${m.from === "user" ? "bg-[#0d6b7a] backdrop-blur text-white rounded-br-md" : "bg-white/75 backdrop-blur border border-white/60 text-ink-body rounded-bl-md shadow-sm"}`}>
              {m.text}
              {m.link && (
                <Link to={m.link[0]} className="block mt-1.5 text-[12.5px] font-bold text-[#0d6b7a] hover:underline">
                  {m.link[1]} →
                </Link>
              )}
              {m.action === "counsellor" && (
                <button onClick={() => onEnquire?.("Chatbot enquiry")} className="block mt-1.5 text-[12.5px] font-bold text-[#0d6b7a] hover:underline">
                  Request a callback →
                </button>
              )}
            </div>
      ...
      <div className="px-3 pt-2.5 flex flex-wrap gap-1.5 bg-white/50 backdrop-blur border-t border-white/60">
        {QUICK.map((q) => (
          <button
            key={q}
            onClick={() => send(q)}
            className="rounded-full border border-white/60 bg-white/60 backdrop-blur px-3 py-1.5 text-[12px] font-semibold text-slate-600 hover:border-[#b7dfe8] hover:text-[#0d6b7a] transition"
          >
            {q}
          </button>
        ))}
      ...
      >
        <input
          ...
          className="flex-1 min-w-0 rounded-xl border border-white/60 bg-white/70 backdrop-blur px-3.5 py-2.5 text-[13.5px] outline-none focus:border-[#0d6b7a] focus:bg-white/90 transition placeholder:text-slate-400"
        />
        <button
          type="submit"
          aria-label="Send message"
          className="w-10 h-10 shrink-0 grid place-items-center rounded-xl bg-[#d4a85d] hover:bg-[#e2bb72] backdrop-blur border border-[#f0deaf]/60 text-[#0d171d] transition"
        >
          <FiSend size={15} />
        </button>
      </form>
